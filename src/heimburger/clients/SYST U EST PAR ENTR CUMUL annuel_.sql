SELECT HISTO_FACTURES.NUMFAC,
       ...
       BPCUSTOMER.BPCNAM_0,
       SALESREP.REPNAM_0,
       DECODE(CODECLI, '000624', 'MULHOUSE', 'RUMILLY')        AS ENTREPOT
FROM (((HISTO_FACTURES LEFT JOIN ARTICLES ON HISTO_FACTURES.REFART = ARTICLES.ARTNUM) LEFT JOIN BPCUSTOMER ON HISTO_FACTURES.CODECLI = BPCUSTOMER.BPCNUM_0) LEFT JOIN ZFAMCLI ON BPCUSTOMER.BPCNUM_0 = ZFAMCLI.CODE_0)
         LEFT JOIN SALESREP ON BPCUSTOMER.REP_0 = SALESREP.REPNUM_0
WHERE ((HISTO_FACTURES.DATEFAC BETWEEN to_date('01/01/2018', 'DD/MM/YYYY') AND to_date('31/08/2018', 'DD/MM/YYYY')
    ...
    AND ((BPCUSTOMER.BPCNUM_0) In ('000624', '000630')))
union all
SELECT ZCA_HEIMBURGER.NUMFAC,
       ...
       BPCUSTOMER.BPCNAM_0,
       SALESREP.REPNAM_0,
       DECODE(ZCA_ADR_SYSTEMEU.BPAADD_0, 'L01', 'MULHOUSE', 'RUMILLY') AS ENTREPOT
FROM ((((ZCA_HEIMBURGER LEFT JOIN ARTICLES ON ZCA_HEIMBURGER.CLE = ARTICLES.ARTNUM) LEFT JOIN BPCUSTOMER ON ZCA_HEIMBURGER.CODECLI = BPCUSTOMER.BPCNUM_0) LEFT JOIN ZFAMCLI ON BPCUSTOMER.BPCNUM_0 = ZFAMCLI.CODE_0) LEFT JOIN SALESREP ON BPCUSTOMER.REP_0 = SALESREP.REPNUM_0)
         INNER JOIN ZCA_ADR_SYSTEMEU ON ZCA_HEIMBURGER.NUMFAC = ZCA_ADR_SYSTEMEU.NUM_0
WHERE ((ZCA_HEIMBURGER.DATEFAC BETWEEN to_date('01/01/2018', 'DD/MM/YYYY') AND to_date('31/08/2018', 'DD/MM/YYYY')
    OR ZCA_HEIMBURGER.DATEFAC BETWEEN to_date('01/01/2019', 'DD/MM/YYYY') AND to_date('31/08/2019', 'DD/MM/YYYY')) AND
       ((ZCA_HEIMBURGER.TYPEDOC) <> 'FP') AND ((BPCUSTOMER.bpcnum_0) = '000624'))
*