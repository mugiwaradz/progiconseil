...
FROM
    ...
    JOIN bpcustomer ON histo_factures.codecli = bpcustomer.bpcnum_0 ) left
    JOIN zfamcli ON bpcustomer.bpcnum_0 = zfamcli.code_0
...
WHERE (HISTO_FACTURES.DATEFAC BETWEEN to_date(?1, 'DD/MM/YYYY') AND to_date(?2, 'DD/MM/YYYY')
    ...
    AND bpcustomer.tsccod_2 = '107'
UNION ALL
SELECT
    zca_heimburger.numfac,
...
FROM
    ...
    JOIN bpcustomer ON zca_heimburger.codecli = bpcustomer.bpcnum_0 ) left
    JOIN zfamcli ON bpcustomer.bpcnum_0 = zfamcli.code_0
